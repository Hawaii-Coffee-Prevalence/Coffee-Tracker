  ...
  return (
    <div className="ghost-surface hover:!bg-base-100 p-7 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div className="text-label text-secondary">Supply Chain Pipeline</div>
        <div className="text-xs text-secondary">{stats?.verifiedCount ?? "—"} verified</div>
      </div>
      ...
      <div className="flex flex-col gap-2">
          ...
          return (
            <div key={seg.key} className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-secondary w-7 text-right">{percent}%</span>
              </div>
            </div>
          );
        })}
        <div className="flex items-center justify-between pt-2 border-t border-base-300">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-secondary">{stats?.verifiedCount ?? "—"}</span>
            <span className="text-xs text-secondary w-7 text-right">
...
const RegionCard = ({ stats, isLoading }: CardProps) => (
  <div className="ghost-surface hover:!bg-base-100 p-7 flex flex-col gap-4">
    <div className="flex items-center justify-between">
      <div className="text-label text-secondary">Batches By Region</div>
      <div className="text-xs text-secondary">{stats?.regionCounters?.length ?? "—"} regions</div>
    </div>
...
const ScaCard = ({ stats, isLoading }: CardProps) => (
  <div className="ghost-surface hover:!bg-base-100 p-7 flex flex-col gap-4">
    <div className="flex items-center justify-between">
      <div className="text-label text-secondary">SCA Score Distribution</div>
      <div className="text-xs text-secondary">Specialty ≥ 80</div>
    </div>
    ...
    <div className="flex flex-col gap-2 my-auto">
      ...
      <div className="grid grid-cols-4 gap-px overflow-hidden">
        {[
          ...
          { label: "Average", value: stats?.averageScaScore ?? "—", cls: "text-amber" },
          { label: "Lowest", value: stats?.lowestSca ?? "—", cls: "text-accent" },
          { label: "Scored", value: stats?.scaBuckets?.reduce((s, b) => s + b.count, 0) ?? "—", cls: "text-secondary" },
        ].map(({ label, value, cls }) => (
          <div key={label} className="px-2 py-2.5 flex flex-col items-center gap-0.5">
            ...
            <span className="text-[0.58rem] font-medium tracking-[0.15em] uppercase text-secondary">{label}</span>
          </div>
        ))}
      </div>
    </div>
