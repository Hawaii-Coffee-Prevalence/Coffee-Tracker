  ...
  const renderTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-base-100 border border-base-300 rounded-lg px-3 py-1 text-xs whitespace-nowrap">
        ...
        <span className="text-secondary ml-2">{payload[0].value} batches</span>
  ...
  return (
    <ResponsiveContainer width="100%" height={100}>
      <BarChart data={data} barSize={18} margin={{ top: 4, right: 0, bottom: 0, left: 0 }}>
        <XAxis
          dataKey="score"
          tick={{ fontSize: 10, fontFamily: "Outfit", fill: "#7a5c3a" }}
          axisLine={false}
          tickLine={false}
        />
        <YAxis hide />
        <Tooltip content={renderTooltip} cursor={{ fill: "#f5ecd7" }} />
