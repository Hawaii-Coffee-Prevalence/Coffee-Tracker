...
export default function DistributeForm() {
  ...
  // Filter for roasted batches (roaster is not zero address)
  const availableBatches =
    batches?.filter(batch => batch.roaster !== "0x0000000000000000000000000000000000000000") || [];
  ...
  return (
    <form onSubmit={handleSubmit} style={formStyle}>
      <div style={{ marginBottom: 16 }}>
        <label style={labelStyle}>Select Batch to Distribute</label>
        <select
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 16 }}>
        <button type="submit" disabled={isMining || status === "sending" || !selectedBatchId} style={btnStyle}>
          {isMining || status === "sending" ? "Submitting..." : "Mark as Distributed"}
        </button>
        {status === "success" && (
          <span style={{ color: colors.greenDark, fontWeight: 600 }}>Marked as distributed ✓</span>
        )}
